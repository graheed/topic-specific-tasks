...
public class CrazyLambdas {
    ...
    public static BiFunction<IntUnaryOperator, IntPredicate, IntUnaryOperator> functionToConditionalFunction() {
        return (intOp, intPred) -> {
            return value -> {
                if (intPred.test(value)){
                    return intOp.applyAsInt(value);
                }
                return value;
            };
        };
    ...
    public static BiFunction<Map<String, IntUnaryOperator>, String, IntUnaryOperator> functionLoader() {
        return (opMap, string) -> {
            if (opMap.containsKey(string)) {
                return opMap.get(string);
            } else {
    ...
    public static <T, U extends Comparable<? super U>> Comparator<T> comparing(Function<? super T, ? extends U> mapper) {
        return (p1, p2) -> mapper.apply(p1).compareTo(mapper.apply(p2)) ;
    }
    ...
    public static <T, U extends Comparable<? super U>> Comparator<T> thenComparing(
        ...
        return (p1, p2) -> comparator.compare(p1, p2) == 0 ? mapper.apply(p1).compareTo(mapper.apply(p2)) : comparator.compare(p1, p2);
